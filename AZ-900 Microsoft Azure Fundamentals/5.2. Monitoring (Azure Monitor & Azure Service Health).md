...
- fundamental data stores: Logs and Metrics
...
- Possible Insights: AJAX calls, session counter, request rates, Host Diagnostics
...
- Visualize in Dashboards, PowerBI, Views, Workbooks

### Analyze

- Analysis in Metric Analytics and Log Analytics
- Log Analytics
  - edit and run query logs using KQL
...
- Alert rule defines who should we monitor and when we need to react
  - Resource: the monitored resource
  - Signal: emits a Signal (Metric, Log, Activity Log, Application Insights)
  - Criteria / Logic
  - Action Group: what to do
  - Monitor Conditon: containing the Alert State
...
- integrate with other services like Logic Apps and Export APIs
